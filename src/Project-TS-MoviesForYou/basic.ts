...
const allGenres = Array.from(new Set(movies.flatMap(movie => movie[4]))).sort();

// display movies
function displayMovies(moviesToShow: Movie[]) {
    ...
    if (moviesToShow.length === 0) {
        movieList.innerHTML = '<div class="no-results">Film not found</div>';
        return;
    }
    ...
    moviesToShow.forEach(movie => {
        ...
        // 2. array-loop f Genres
        let genresHTML = '<p>Genres: ';
        genres.forEach((genre, index) => {
            genresHTML += genre;
            if (index < genres.length - 1) genresHTML += ', ';
        });
        genresHTML += '</p>';
...
function searchMovies() {
    ...
    if (selectedGenre) {
        filteredMovies = filteredMovies.filter(movie => movie[4].includes(selectedGenre));
    }
...
function addMovie() {
    ...
    const genresInput = (document.getElementById('newGenres') ).value;
    const rating = (document.getElementById('newRating') ).value;

    if (title && year && director && duration && genresInput && rating) {
        ...
        // update Genres 
        const genreFilter = document.getElementById('genreFilter');
        const newGenres = Array.from(new Set(movies.flatMap(movie => movie[4]))).sort();
        genreFilter.innerHTML = '<option value="">Alle Genres</option>' +
            newGenres.map(genre => `<option value="${genre}">${genre}</option>`).join('');

        // leer
        (document.querySelectorAll('.add-movie-input input') as NodeListOf<HTMLInputElement>).forEach(input => input.value = '');
...
function init() {
    ...
    const bestRateBtn = document.getElementById('bestRateBtn') ;
    const addMovieBtn = document.getElementById('addMovieBtn') ;

    if (!searchInput || !searchBtn || !genreFilter || !yearUpBtn || !yearDownBtn || !bestRateBtn || !addMovieBtn || !document.getElementById('movieList')) {
    ...
    bestRateBtn.addEventListener('click', sortBestRate);
